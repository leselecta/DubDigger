...
- **The hero belongs to the home page, and a query ends it. Changed 2026-09-09.** One file serves both, and once there is a `?q=` it stops being the home page: the headline was already dropped, and now the hero field goes with it and the header's own field is switched on instead, carrying the query so a reader who wants to change it finds what they typed still in the box. Keeping both would have been the page asking twice for the same thing, which is the rule `search={false}` was written for read the other way round. What is left is the shape every other page has: a column, an eyebrow, a title, the answer under it. **The cost is on a phone**, where the header field is `hidden sm:block` and the drawer carries the only copy, so re-searching went from a field on screen to a tap on the hamburger. Accepted as the site's own pattern rather than fixed, since every other page already works that way, but this is the one page where changing the query is the likeliest next thing a reader does.
