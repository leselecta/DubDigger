...
- **The search box pivots too, and it is one click from any page.** The suggestion dropdown is the shortest version of the same move: type, arrow down, Enter, and you are on the page without the results list in between. Its rows carry the grade in the same five words the results column uses, because a shortlist that ranked names and said nothing about why would be ordering by a measure it kept to itself. It orders them exactly as the results page orders the same names, since it is a shortcut into that page and not a second opinion about it. A miss returns nothing and the dropdown stays shut: the honest answer to a miss is the one the results page gives in full, that the corpus is a slice centred on dub techno and a legitimate absence is the boundary working, and Enter still goes there.
