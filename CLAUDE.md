...
- **A sort reorders the list, it never rebuilds it.** Core Labels took a four-cell control on 2026-09-03: ranking, A–Z, releases, active. The list is still the top `TOP_LIST_SIZE` by scene weight, and every other ordering is a different view of that same set rather than a reach into the corpus for the 803 oldest labels or the 803 with the most records, which would be a different page wearing this one's heading. Ranking is a cell in the row rather than an implied state, which is what makes the other three toggles: the way back is the cell next to the one you clicked. They take the column names verbatim, since a control that renamed "Releases" or "Active" would be a second vocabulary for one question, and the direction is left to the ordering to show: most records first, oldest first, and a label with no years at all sorts last rather than reading as the oldest on the site. Server side and in the URL like the tabs, so `?sort=active` is an address. **Core Artists is still the old pair behind `SHOW_SORT`**, deliberately: labels first, then the same three there.
