...
- **Display type is fluid, reading type is fixed.** `--text-hero`, `--text-name`, `--text-name-label` and `--text-stat` are clamps, because the handoff's pixel sizes are wider than a phone (a 104px "Moritz von Oswald" needs 900px of viewport). `--text-row`, `--text-lead` and `--text-body` are fixed. Add a new size only if the design has one.
  ...
  Prose reads at two of those, and the split is by job rather than by page. `--text-lead` (1.1875rem) is the paragraph directly under a headline, a subheading doing a headline's work. `--text-body` (1.0625rem) is prose you settle into: a bio, the bands on the About page. Both carry their line height on the token itself, so a paragraph asks for a size and gets the leading that belongs to it. Do not respell it with a `leading-*` class.
...
**Client JavaScript is 17 KB on every page, and that is the budget.** Nearly all of it is `ClientRouter` at 16,075 bytes, which is deliberate: the premise is that a pivot costs one click, so the document is swapped rather than reloaded. The rest is two inline scripts measured in hundreds of bytes, the collapsing bio at 658 and the scroll hold at 300. Anything you add comes out of a budget with three line items, and a fourth needs the same argument those three made.

**Prefer the server, then a link, then a script.** Tabs, pagination and search are links and a plain GET form carrying state in the URL. That is what keeps a 556 row roster from being serialised into the page as JSON, and what makes every view a real address. Only reach for a script when the answer genuinely is not knowable on the server: the collapsing bio qualifies, because whether the text overflows depends on the rendered line count at this viewport, and the scroll hold qualifies, because where the reader was is not something a server response can carry. A fragment was tried there first and could only say "put the tab bar at the top", which still moves the page. It is one script in the layout, keyed on `data-hold-scroll`, serving both the tab bar and "Load more": the same script twice would cost the budget twice.
