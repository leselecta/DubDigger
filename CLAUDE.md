...
- **Two families, split by job.** Helvetica Neue for names and headlines. IBM Plex Mono, uppercase and letterspaced, for everything structural: labels, counts, roles, meta, controls. The `mono-label` utility is that pattern written down, so use it rather than respelling it.
  ...
  **Helvetica Neue is only installed on Apple platforms, and until 2026-08-16 the stack just shrugged at that.** Windows fell to Arial and Android to Roboto, measured 6–7% narrower than the design on every string tested, at up to 104px. So `--font-sans` now leads with `HN Local`, a `local()`-only face that resolves the installed Helvetica Neue and downloads nothing, then falls through to `DubDigger Sans`: a renamed subset of TeX Gyre Heros, a genuine Helvetica clone within 0.88% of Neue on width. 36.4 KB for two weights, paid only off Apple platforms. Its one visible tell is that Heros clones Helvetica's bold, which is lighter than Neue's.

  **Two families rather than one face with two sources, and that is not stylistic.** `ascent-override` applies to the whole `@font-face` including a `local()` match, so a single shared face would rewrite Helvetica Neue's own metrics on a Mac. Heros ships an ascent of 114.8% against Neue's 95.2%, so the override is needed and has to be quarantined to the faces that need it. `local()` also matches on full name or PostScript name and never the family name, which is why the bold face names `HelveticaNeue-Bold`. Rebuild the subset with `web/scripts/subset-heros.py`; the reasoning and the licence are in `web/public/fonts/README.md`.
